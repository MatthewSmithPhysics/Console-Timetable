...
- **superHTML** [daymin] [daymax] [timemin] [timemax] [directory] [filename]; Exports the current timetable to *fileroot*_*j*.html in *directory* for every week number j, from *daymin* (Mon to Sun) to *daymax* (Mon to Sun) and from *timemin*:00 to *timemax*:00.
